...
export const config = {
     ...
     */
    before: async function setarApp() {
        ...
        await logo.waitForDisplayed({interval: 5000});
        for (let numloop = 0; numloop <= 11; numloop++) {
            await logo.click();
        }
        const senhaAmbiente = await driver.$("class name:android.widget.EditText");
        await senhaAmbiente.waitForDisplayed();
        ...
        const selecionaHml = await driver.$("-android uiautomator:new UiSelector().className(\"android.widget.Image\").instance(2)");
        await selecionaHml.waitForDisplayed();
        await selecionaHml.click();
        const allowAnalitycs = await driver.$("//android.view.View[@resource-id='ui-toggle__btn--toggle']/android.widget.Button");
        await allowAnalitycs.click();
        const confirmaselecaoAmbiente = await driver.$("(//android.widget.Image[@resource-id='ui-icon__img--icon-arrow-medium-right'])[2]");
        ...
        await acessesuaConta.waitForDisplayed({interval: 5000});
